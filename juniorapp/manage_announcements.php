...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['delete'])) {
        ...
        $is_new_announcement = !isset($_POST['id']);
        ...
        // Send email notification only for new announcements
        if ($is_new_announcement) {
            // Get all users who should receive the announcement
            $stmt = $conn->prepare("SELECT email_address, username FROM users WHERE receive_announcements = 1");
            $stmt->execute();
            $result = $stmt->get_result();

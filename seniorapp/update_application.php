...
try {
    ...
    if ($result->num_rows === 0) {
        throw new Exception("No existing application found to update.");
    }
    ...
    if (!$stmt->execute()) {
        throw new Exception("Failed to update application. Please try again. " . $stmt->error);
    }
    $stmt->close();

    // Delete existing files
    $stmt = $conn->prepare("DELETE FROM application_files WHERE application_id = ?");
    if (!$stmt) throw new Exception("Database error: " . $conn->error);
    $stmt->bind_param("i", $application_id);
    if (!$stmt->execute()) {
        throw new Exception("Failed to clear old files. " . $stmt->error);
    }
    $stmt->close();

...
// Fetch reviewers
$reviewers = [];
$res = $conn->query("SELECT id, username FROM users WHERE role='reviewer'");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['assign'])) {
    ...
    foreach ($assignments as $reviewer_id => $count) {
        ...
        if ($count > 0) {
            ...
            $stmt = $conn->prepare("SELECT id FROM applications WHERE (assigned_to IS NULL OR assigned_to = '') AND status = ? ORDER BY RAND() LIMIT ?");
            ...
            foreach ($app_ids as $app_id) {
                ...
                $update->bind_param("si", $reviewer_id, $app_id);
                $update->execute();
                $update->close();
            }
        }
    }
    $success = "Applications assigned successfully!";
}
...
<head>
    ...
    <style>
        ...
        .success-msg { color: #388e3c; font-weight: bold; margin-bottom: 15px;}
